---
name: frontend
description: Rules for the local Next.js viewer and control surface under frontend/.
paths:
  - "frontend/**"
---
...
- Keep the two sources honest. NASA is published transit depth (the real science input); MAST is
  ...
  must keep the distinction obvious. This is the upper-limits framing in [GEMINI.md](../../GEMINI.md)
  applied to the interface; see also [documentation.md](documentation.md) for reachability.
- Data hygiene. Everything the app downloads or uploads under `data/` is bulk data and stays
  git-ignored; the root `.gitignore` already covers `data/*/raw/`, `mastDownload/`, and the generated
  per-point CSVs. Never commit those, and never commit `frontend/node_modules` or `frontend/.next`.
  Background: [data.md](data.md).
